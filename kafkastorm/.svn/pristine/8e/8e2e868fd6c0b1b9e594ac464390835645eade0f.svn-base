...
public class diskUsedPercent extends associatedData{
	String SEPARATOR = "#$#";
	public diskUsedPercent(String gatherid){
	...
	public String[] getcalData() {
		...
		for (Entry<String, HashMap<String,String>> entry : rr.entrySet()) {
			 if(entry.getValue().get("1.3.6.1.2.1.25.2.3.1.2."+entry.getKey()).equals("1.3.6.1.2.1.25.2.1.3")){
		...
		BigDecimal memusedpercent = used.divide(total, 2, RoundingMode.HALF_DOWN);
