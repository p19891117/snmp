...
public class sendRcvInterface extends associatedData{
	String SEPARATOR = "#$#";
	public sendRcvInterface(String gatherid){
	...
	public String[] getcalData() {
		...
		BigDecimal indivtotal = new BigDecimal(0);
		...
		for (Entry<String, HashMap<String,String>> entry : rr.entrySet()) {
			 //if(entry.getValue().get("1.3.6.1.2.1.25.2.3.1.2."+entry.getKey()).equals("1.3.6.1.2.1.25.2.1.4")){
				 ...
				 BigDecimal mi = new BigDecimal((String)this.caloidvalue.get(this.caldoid.indexOf("1.3.6.1.2.1.2.2.1.20")));
				 BigDecimal mj = new BigDecimal((String)this.caloidvalue.get(this.caldoid.indexOf("1.3.6.1.2.1.2.2.1.14")));
				 subsummary = subsummary.add(mi.subtract(mj));
				 indivtotal = indivtotal.add(mi);
				 
				 
			// }
		 }
		 BigDecimal pp = subsummary.divide(indivtotal, 2, RoundingMode.HALF_DOWN);
