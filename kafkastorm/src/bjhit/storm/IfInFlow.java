  ...
  String SEPARATOR = "#$#";
	...
	public String[] getcalData() {
		...
		if(this.calculateAccesibleNext == this.calculateInNeedNext){
			...
			BigDecimal inTimeDelta = new  BigDecimal(0);
			try{
				...
				for (Entry<String, HashMap<String,String>> entry : rr.entrySet()) {
					...
					inOctets = inOctets.add(new BigDecimal((String)this.caloidvalue.get(this.caldoid.indexOf("1.3.6.1.2.1.2.2.1.10"))));
				...
				for (Entry<String, HashMap<String,String>> entry : rrNext.entrySet()) {
					...
					inOctetsNext = inOctetsNext.add(new BigDecimal((String)this.caloidvalueNext.get(this.caldoid.indexOf("1.3.6.1.2.1.2.2.1.10"))));
			...
			try {
				...
				inTimeDelta = new BigDecimal(timeDelta/1000);
				BigDecimal inFlow = inOctetsNext.subtract(inOctets).divide(inTimeDelta, 2, RoundingMode.HALF_DOWN);
				this.calValue = String.valueOf(inFlow);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				System.out.println("IfInFlow "+e.toString());
				e.printStackTrace();
			}
			...
			//
			 ...
			 System.out.println("1.3.6.1.2.1.2.2.1.10 Nextvalue is "+this.caloidvalueNext.get(this.caldoid.indexOf("1.3.6.1.2.1.2.2.1.10")));
			 ...
			 System.out.println("IfInFlow END");
			 //reset
			 this.calculateAccesibleNext = 1;
			 this.calValue = "-1";
		}
