...
public class diskTotal extends associatedData{
	String SEPARATOR = "#$#";
	public diskTotal(String gatherid){
	...
	public String[] getcalData() {
		...
		//BigDecimal mj;
		//BigDecimal mk;
		BigDecimal memtotal = new BigDecimal(0);
		...
		for (Entry<String, HashMap<String,String>> entry : rr.entrySet()) {
			 if(entry.getValue().get("1.3.6.1.2.1.25.2.3.1.2."+entry.getKey()).equals("1.3.6.1.2.1.25.2.1.4")){
				 ...
				 memtotal = memtotal.add(memused);
				 
			 }
		 }
		 this.calValue = String.valueOf(memtotal);
		 //
		 Date date=new Date();  
