...
public class memUsed extends associatedData{
	String SEPARATOR = "#$#";
	public memUsed(String gatherid){
	...
	public String[] getcalData() {
		...
		/*if(this.tup == null)
			return null;
		String[] kafkadata = this.tup.getString(0).split(SEPARATOR);
		this.Batch = Integer.valueOf(kafkadata[0]);
		this.bTime = kafkadata[1];
		this.IP = kafkadata[2];*/
		...
		for (Entry<String, HashMap<String,String>> entry : rr.entrySet()) {
			 if(entry.getValue().get("1.3.6.1.2.1.25.2.3.1.2."+entry.getKey()).equals("1.3.6.1.2.1.25.2.1.2")){
				 ...
				 this.calValue = String.valueOf(memused);
				 break;
			 }
		 }
		 //
		...
		/* String[] ret = new String[this.caldoid.size()+1];
		 for(int i=0;i<this.caldoid.size();i++){
			 ret[i] = String.valueOf(this.Batch)+SEPARATOR+this.bTime+SEPARATOR+
		 this.IP+SEPARATOR+this.caldoid.get(i)+SEPARATOR+this.caldoid.get(i)+SEPARATOR+
					 this.caloidvalue.get(i)+SEPARATOR+this.gtime;
		 }
		 ret[this.caldoid.size()] = String.valueOf(this.Batch)+SEPARATOR+this.bTime+SEPARATOR+
				 this.IP+SEPARATOR+this.scn_id+SEPARATOR+this.scn_id+SEPARATOR+
				 this.calValue+SEPARATOR+this.gtime;
		return ret;*/
	}
	@Override
