...
<script type="text/javascript"
	...
	<script src="<%=request.getContextPath()%>/js/ht/playAxisll.js"></script>
<script type="text/javascript"
...
<!--[if lt IE 9]>
	...
	<script type="text/javascript">
	    ...
	    var sceneInfoList=${sceneInfoList};
	    var index=0;
	    var rapid;
	    var flag = 0
		...
		function addInfo(sceneInfo){ 
		    ...
		    if(sceneInfo != "1" && flag ===0){
				...
				innerHtml += "<td class='txt_'  width='5%'>";
				innerHtml += (index + 1);
				innerHtml += "</td><td width='10%'>";
				...
				innerHtml += "</td><td width='9%'><a href='javascript:void(0);' onClick='listSceneBack(\""+sceneInfo.scn_id+"\");'><img src='<%=request.getContextPath()%>/images/icn_categories.png' title='查看' class='vm'> 查看</a>";
				innerHtml += "</td></tr>";
...
<script type="text/javascript"> 
   ...
   setTimeout(chart.playNow(),2000);
	...
	function getPlayAxisData(){
		var result = "";
		$.ajax({
            url : '../verTaskctrl/getPlayAxisData',
            data:{"taskid":'${task.id}'},
            type : 'post',
            async: false,//同步
            dataType:"json",
