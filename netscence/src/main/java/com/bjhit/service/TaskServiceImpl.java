...
public class TaskServiceImpl {
    ...
    public Pager getTaskList_hs(Task task,Pager p) {
        //总记录数
        p.setTotalCount(taskMapper.getTaskCount_hs(task));
        p.setItems(taskMapper.getTaskList_hs(task));
    ...
    public int saveFindConfig(ScenceFindConfigVo vo) {
        ...
        if(vo.getValue() != null && !"".equals(vo.getValue())){
        	if("1".equals(vo.getFlag())){
        		List<TopoDevice> topoDevices = taskMapper.topoDevicesByTaskANDCategary(vo);
        		ScenceFindConfigVo tmp = new ScenceFindConfigVo();
        		tmp.setCompare_type(vo.getCompare_type());
        		tmp.setCompare_type_name(vo.getCompare_type_name());
        		tmp.setDev_type(vo.getDev_type());
        		tmp.setFlag(vo.getFlag());
        		tmp.setGather_id(vo.getGather_id());
        		tmp.setGather_name(vo.getGather_name());
        		tmp.setId(vo.getId());
        		tmp.setIp(vo.getIp());
        		tmp.setRegular_type(vo.getRegular_type());
        		tmp.setRegular_type_name(vo.getRegular_type_name());
        		tmp.setTask_id(vo.getTask_id());
        		tmp.setValue(vo.getValue());
        		for(TopoDevice t:topoDevices){
        			if(!t.getIp().equals(vo.getIp())){
        				tmp.setIp(t.getIp());
        				result = taskMapper.saveFindConfig(tmp);
        			}
        		}
        	}
	...
	public int saveCopyTask(Task task , String id){
		...
		List<ScenceFindConfigVo> findlist = taskMapper.getFindConfigList(id, "");
		for(int i=0; i<findlist.size(); i++){
		    ScenceFindConfigVo vo = findlist.get(i);
		    vo.setTask_id(taskid);
		    taskMapper.saveFindConfig(vo);
		}
		return taskid;
	}
}
